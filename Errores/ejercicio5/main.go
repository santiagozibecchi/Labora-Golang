...
1. **Estructura del Estudiante**:
    - Define una estructura **`Estudiante`** que tenga al menos dos campos: **`Nombre`** (string) y **`ID`** (int) y 
	`**calificaciones**` (mapa) con llave siendo la materia (string) y el valor siendo la nota (float)
...
5. **Funciones con Manejo de Errores**:
    - Modifica las funciones para añadir, actualizar, y eliminar estudiantes para que manejen situaciones como intentar actualizar un estudiante
	que no existe.
